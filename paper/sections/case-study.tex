...
vibe-coding pitches (T2, T4, T5) leaned on iterative prompting, while
engineering-led teams (T1, T7--T12) opened with explicit planning
artifacts. \textbf{Single- versus multi-agent} use trended toward
single-agent loops; only T12 (this study) fanned parallel subagents
from a decomposed plan. \textbf{Artifact production} was correspondingly
uneven across the two clusters. Without controlled comparison we
cannot link these patterns to outcomes, but the field samples the
preparation-versus-iteration spectrum motivating RQ1 and the
